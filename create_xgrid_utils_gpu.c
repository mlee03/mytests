...
void get_grid_cell_struct_gpu_(int *nlon, int *nlat, double *lon, double *lat,
                               double *lon_min, double *lon_max, double *lat_min, double *lat_max, double *lon_cent,
                               double *area, int *nvertices, double *lon_vertices, double *lat_vertices)
{
  get_grid_cell_struct_gpu(nlon, nlat, lon, lat, lon_min, lon_max, lat_min, lat_max, lon_cent,
                           area, nvertices, lon_vertices, lat_vertices);
}

void get_grid_cell_struct_gpu(int *nlon, int *nlat, double *lon, double *lat,
                              double *lon_min, double *lon_max, double *lat_min, double *lat_max, double *lon_cent,
                              double *area, int *nvertices, double *lon_vertices, double *lat_vertices)
{
...
#pragma acc enter data create(lon_min[:ncells], lon_max[:ncells], \
                              ...
                              area[:ncells], lon_vertices[:MAX_V*ncells], \
                              lat_vertices[:MAX_V*ncells])
#pragma acc data present(lon[:npts], lat[:npts])
#pragma acc parallel loop independent
  for(int icell=0; icell<ncells; icell++){
    int nverticies_i;
    double lon_vertices_i[MV], lat_vertices_i[MV];

    get_cell_vertices_gpu( icell, *nlon, lon, lat, lon_vertices_i, lat_vertices_i );

    lat_min[icell] = minval_double_gpu(4, lat_vertices_i);
    lat_max[icell] = maxval_double_gpu(4, lat_vertices_i);
    
    nverticies_i = fix_lon_gpu(lon_vertices_i, lat_vertices_i, 4, M_PI);
    nvertices[icell] = nverticies_i;

    if(nverticies_i>MAX_V) printf("ERROR get_cell_minmaxavg_latlons:  number of cell vertices is greater than MAX_V\n");
    lon_min[icell]  = minval_double_gpu(nverticies_i, lon_vertices_i);
    lon_max[icell]  = maxval_double_gpu(nverticies_i, lon_vertices_i);
    lon_cent[icell] = avgval_double_gpu(nverticies_i, lon_vertices_i);
    
    area[icell] = poly_area_gpu(lon_vertices_i, lat_vertices_i, nverticies_i);

    for(int ivertex=0 ; ivertex<nverticies_i ; ivertex++) {
      lon_vertices[MAX_V*icell+ivertex] = lon_vertices_i[ivertex];
      lat_vertices[MAX_V*icell+ivertex] = lat_vertices_i[ivertex];
    }
  }
...
void get_cell_vertices_gpu( int icell, int nlon, double *lon, double *lat, double *x, double *y )
{
